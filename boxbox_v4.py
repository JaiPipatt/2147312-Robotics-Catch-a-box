...
def find_best_pair(lines, img_height, img_width):
    """
    Evaluate all perpendicular line pairs jointly and pick the one where
    both lines are long and their endpoints meet closest at a corner.
    """
...
def merge_parallel_connected_lines(perp_line, lines):
    """Extend a line by absorbing nearby parallel segments connected at its ends."""
    ...
    for line in lines:
        ...
        angle_diff = abs(np.degrees(line_angle - perp_angle)) % 180
        angle_diff = min(angle_diff, 180 - angle_diff)
    ...
    if len(all_points) > 2:
        ...
        return tuple(all_points[np.argmin(distances)]) + tuple(all_points[np.argmax(distances)])
...
def visualize_result(result):
    ...
    # Draw all detected lines in random colours
    np.random.seed(42)
    ...
    # Highlight the winning pair and key points
    cv2.line(img_copy,   (x1,  y1),  (x2,  y2),  (0, 255, 0),   3)   # green  = best line
    cv2.line(img_copy,   (px1, py1), (px2, py2), (255, 0, 255),  3)   # magenta = perp line
    cv2.circle(img_copy, (int(corner_x), int(corner_y)),          8, (255, 0,   0),   -1)  # blue   = corner
    cv2.circle(img_copy, (int((x1+x2)/2),  int((y1+y2)/2)),       5, (0,   0,   255), -1)  # red    = edge midpoints
    cv2.circle(img_copy, (int((px1+px2)/2), int((py1+py2)/2)),    5, (0,   0,   255), -1)
    cv2.circle(img_copy, (int(center_x), int(center_y)),          6, (0,   255, 255), -1)  # cyan   = center
