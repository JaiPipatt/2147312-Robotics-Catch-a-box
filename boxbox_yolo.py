...
def detect(img, model=None):
    """
    Detect a pink box in img (BGR numpy array).

    Returns a result dict or None if no box found.
    Keys: center, orientation, box_pts, mask, img, mode ('yolo' | 'color')
    """
    img   = undistort(img)
        ...
        if best_box is not None and best_pink > 0.05:
            ...
            if contours:
                ...
                if cv2.contourArea(cnt) > MIN_CONTOUR_AREA * 0.1:
                    # Offset: ROI offset + box crop offset
                    res = contour_to_result(cnt, offset_xy=(ox + bx1, oy + by1))
                    # Build full mask for display
...
def run_realtime(cam_index=CAM_INDEX, model=None):
    ...
    # Create detection window with HSV trackbars
    ...
    while True:
        ...
        if result is not None:
            ...
            status_txt = "PINK BOX DETECTED"
            status_col = (0, 255, 0)
            pos_mm  = result.get('pos_mm')
            partial = result.get('partial', False)
            if pos_mm and not partial:
                with _latest_lock:
                    _latest['valid']  = True
            ...
            status_txt = "No pink box"
            status_col = (0, 0, 255)
            with _latest_lock:
                _latest['valid'] = False

            # Show raw pink mask even when no box found
        ...
        cv2.putText(frame, f"FPS: {fps:.1f}  |  S=save HSV  Q=quit",
                    (10, frame.shape[0] - 15),
